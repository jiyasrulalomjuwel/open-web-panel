...
func getOrCreateAccountKey(db *sql.DB) crypto.Signer {
	...
	if err != nil {
		log.Printf("[ACME] Generate account key error: %v", err)
		return nil
	}

	keyDER, err := x509.MarshalPKCS8PrivateKey(key)
	if err != nil {
		log.Printf("[ACME] Marshal account key error: %v", err)
		return nil
	}
...
func issueLetsEncryptCert(db *sql.DB, certID int64, domains ...string) error {
	...
	if accountKey == nil {
		return fmt.Errorf("acme: no account key available")
	}
	client := &acme.Client{
		Key:          accountKey,
...
// It looks up the account's PHP-FPM socket so the HTTPS block uses the correct PHP version.
func addNginxSSL(db *sql.DB, dom string) {
	...
	// Look up the account's PHP-FPM socket for this domain
	...
	db.QueryRow(`SELECT COALESCE(pv.socket_path, '') FROM account_php_version apv
		...
		JOIN domains d ON d.account_id = apv.account_id
		WHERE d.domain = ?`, dom).Scan(&phpSocket)
	if phpSocket == "" {
		phpSocket = getEnvDefault("PHP_FPM_SOCKET", "/run/php/php8.3-fpm.sock")
	}
	...
	go func() {
		...
		for range ticker.C {
			...
			if err := rows.Err(); err != nil {
				log.Printf("[ACME] renewal rows iteration error: %v", err)
			}
			rows.Close()
		}
	}()
