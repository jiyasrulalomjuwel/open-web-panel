...
func certRoutes(r chi.Router, db *sql.DB) {
	r.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := rows.Err(); err != nil {
			log.Printf("[SSL] rows iteration error: %v", err)
			jsonResp(w, 200, []interface{}{})
			return
		}
	...
	r.Post("/issue", func(w http.ResponseWriter, r *http.Request) {
		...
		if isRAMExceeded(db, c.AccountID) {
			jsonError(w, 429, "RAM limit exceeded. SSL certificate issuance is temporarily blocked. Please contact your hosting administrator to upgrade your resource allocation.")
			return
		}
	...
	r.Post("/custom", func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := db.Exec("DELETE FROM ssl_certs WHERE domain_id = ?", req.DomainID); err != nil {
			log.Printf("[SSL] Failed to delete existing cert for domain_id %d: %v", req.DomainID, err)
		}
	...
	r.Delete("/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		var domain string
		err = db.QueryRow("SELECT domain FROM ssl_certs WHERE id = ?", id).Scan(&domain)
		if err != nil {
			jsonError(w, 404, "cert not found")
			return
		}
