...
func childEmailRoutes(r chi.Router, db *sql.DB) {
	...
	r.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := rows.Err(); err != nil {
			log.Printf("[EMAILS] list rows iteration error: %v", err)
			jsonResp(w, 200, []interface{}{})
			return
		}
	...
	r.Post("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if c != nil && isRAMExceeded(db, c.AccountID) {
			jsonError(w, 429, "RAM limit exceeded. New email account creation is temporarily blocked. Please contact your hosting administrator to upgrade your resource allocation.")
			return
		}
	...
	r.Delete("/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		if affected == 0 {
			jsonError(w, 404, "email account not found")
			return
		}
	...
	if err := rows.Err(); err != nil {
		log.Printf("[EMAILS] messages rows iteration error: %v", err)
		jsonResp(w, 200, []interface{}{})
		return
	}
	jsonResp(w, 200, msgs)
...
// Read a single message
	...
	r.Get("/dns", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := rows.Err(); err != nil {
			log.Printf("[EMAILS] dns rows iteration error: %v", err)
			jsonResp(w, 200, []interface{}{})
			return
		}
...
func adminEmailRoutes(r chi.Router, db *sql.DB) {
	...
	r.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := rows.Err(); err != nil {
			log.Printf("[EMAILS] admin list rows iteration error: %v", err)
			jsonResp(w, 200, []interface{}{})
			return
		}
		jsonResp(w, 200, accts)
