...
var (
	...
	logger     = logging.NewDefault("dbadmin")
)
...
func main() {
	...
	if jwtSecret == "" {
		logger.Errorf("OWP_JWT_SECRET environment variable is not set. This is critical for security.")
		return
	}
	...
	if err != nil {
		logger.Errorf("Failed to open system DB: %v", err)
		return
	}
	...
	logger.Infof("DB Admin Tool running on 127.0.0.1:%s", port)
	// Bind loopback only: the tool exposes tenant DB contents and administrative
	// query access, so it must never be reachable from other interfaces. Admins
	// reach it through the panel's local reverse proxy.
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(path, "api/tables/") {
		...
		handleListTables(w, sanitizeDBName(dbName))
		return
	}
...
func handleQuery(w http.ResponseWriter, r *http.Request) {
	...
	db := getDB(req.DB)
	if db == nil {
		jsonResp(w, 404, map[string]string{"error": "database not found"})
		return
	}

	jsonResp(w, 403, map[string]string{"error": "Direct SQL query execution is disabled for security. Refactor needed."})
}
...
// sanitizeDBName restricts a user-supplied database name to a safe character
// set so it cannot traverse out of the dbDir (path traversal via "api/tables/").
