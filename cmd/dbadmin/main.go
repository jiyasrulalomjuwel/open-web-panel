...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	// Check basic auth (set by pmaProxyHandler)
...
<div class="layout">
  ...
  <div class="main">
    <div class="toolbar">
      ...
      <button onclick="document.getElementById('sqlInput').value='SELECT * FROM sqlite_master'">Show Tables</button>
    ...
    <div id="messages"></div>
    <textarea id="sqlInput" class="sql-editor" placeholder="Enter SQL query..." spellcheck="false">SELECT * FROM sqlite_master</textarea>
...
func handleListDatabases(w http.ResponseWriter) {
	// Get system DB info
	var name string
	row := systemDB.QueryRow("PRAGMA database_list")
	var seq int
	var file string
	...
	// Find child databases from the system
	...
	if err == nil {
		...
		for rows.Next() {
			var dbName string
			rows.Scan(&dbName)
			// Check if there's a corresponding SQLite file
...
func handleQuery(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(upper, "SELECT") || strings.HasPrefix(upper, "PRAGMA") || strings.HasPrefix(upper, "EXPLAIN") {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{"affected": affected})
	}
}
