...
NGINX_CONF
  ...
  run_cmd "Validate nginx configuration" "nginx -t 2>&1" "true"
...
CRON2
  ...
  nginx -t 2>/dev/null && systemctl enable nginx --now 2>/dev/null || true
...
stage_validate() {
  ...
  for port in 80 "${OWP_PANEL_PORT}" "${OWP_USER_PORT}" 9000; do
