...
import { fetchFeatures } from './FeatureGate';

// Child route -> gating feature. Emails stay visible when disabled (read-only
// inbox); everything else listed here is hidden while blocked.
const routeFeatures: Record<string, string> = {
  '/child/files': 'files',
  '/child/ftp': 'ftp',
  '/child/databases': 'db',
  '/child/backups': 'backups',
};
...
const topLinks = [
  ...
  { to: '/child/backups', icon: HardDrive, label: 'Backups' },
];
...
export function ChildLayout() {
  ...
  const isHidden = (to: string) => {
    const f = routeFeatures[to];
    return !!f && features != null && features[f] === false;
  };
    ...
    <div className="flex h-screen bg-surface overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
          {topLinks.filter(({ to }) => !isHidden(to)).map(({ to, icon: Icon, label }) =>
          ...
          {groups.map(g => {
            const visibleChildren = g.children.filter(c => !isHidden(c.to));
            if (visibleChildren.length === 0) return null;
            const groupActive = isGroupActive(visibleChildren.map(c => c.to));
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={profileRef}>
              <button
                ...
                className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#2563EB] to-[#3B82F6] flex items-center justify-center text-white text-xs font-bold shrink-0 hover:opacity-90 transition-opacity"
              ...
              <AnimatePresence>
                {profileOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -8, scale: 0.96 }}
                    transition={{ duration: 0.12 }}
                    className="absolute right-0 mt-2 w-56 bg-white border border-border-subtle rounded-xl shadow-dropdown overflow-hidden z-50"
                  >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2.5 w-full px-4 py-3 text-sm text-gray-500 hover:text-red-600 hover:bg-red-50 transition-colors"
                    >
