...
export function Tickets() {
  ...
  const loadTickets = useCallback(() => {
    setLoading(true);
    setError('');
    getTickets({ limit: 100 }).then(d => { setTickets(ensureArray(d?.tickets || d)); }).catch((e: any) => { setError(e?.error || 'Failed to load tickets'); console.error('Load tickets:', e); }).finally(() => setLoading(false));
  }, []);
  ...
  const loadMessages = async (id: number) => {
    ...
    try {
      ...
      if (inflightTicketId.current === id) { setMessages([]); setError(e?.error || 'Failed to load messages'); }
    }
    finally {
  ...
  const selectedStatus = tickets.find((t) => t.id === selectedTicket)?.status;
  const handleStatus = async (status: 'closed' | 'open') => {
    if (!selectedTicket) return;
    setStatusLoading(true);
    try {
      await updateTicketStatus(selectedTicket, status);
      setSuccess(status === 'closed' ? 'Ticket closed' : 'Ticket re-opened');
      loadTickets();
    } catch (err: any) { setError(err?.error || 'Failed to update ticket'); }
    finally { setStatusLoading(false); }
  };

  const statusBadge = (s: string) => {
    const colors: Record<string, string> = { open: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300', closed: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400', replied: 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300', pending: 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300' };
          ...
          {selectedTicket ? (
            <Card padding={false}>
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-gray-700">
                <span className="text-xs text-gray-500 dark:text-gray-400">Ticket #{selectedTicket}</span>
                {selectedStatus === 'closed' ? (
                  <Button variant="secondary" size="sm" onClick={() => handleStatus('open')} disabled={statusLoading} loading={statusLoading}>Re-open</Button>
                ) : (
                  <Button variant="secondary" size="sm" onClick={() => handleStatus('closed')} disabled={statusLoading} loading={statusLoading}>Close ticket</Button>
                )}
              </div>
              <div className="flex flex-col h-[460px]">
                  ...
                  {msgsLoading ? (
                    ...
                    <div key={m.id} className={`flex ${m.sender_type === 'admin' ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-[80%] rounded-lg px-4 py-2 text-sm ${m.sender_type === 'admin' ? 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200' : 'bg-emerald-600 text-white'}`}>
                        ...
                        <div className="whitespace-pre-wrap break-words max-h-96 overflow-y-auto">{m.message}</div>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 p-3 flex gap-2">
                  <textarea value={replyMsg} onChange={e => setReplyMsg(e.target.value)}
                    ...
                    className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg text-sm dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <textarea value={msg} onChange={e => setMsg(e.target.value)} rows={5}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg text-sm dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  placeholder="Describe your issue in detail..." maxLength={50000} required />
              </div>
...
export function TicketManagement() {
  ...
  const loadTickets = useCallback(() => {
    setLoading(true);
    setError('');
    getAdminTickets({ limit: 200 }).then(d => { setTickets(ensureArray(d?.tickets || d)); }).catch((e: any) => { setError(e?.error || 'Failed to load tickets'); console.error('Load admin tickets:', e); }).finally(() => setLoading(false));
  }, []);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
          {['all', 'open', 'replied', 'pending', 'closed'].map(f => (
            <button key={f} onClick={() => { setFilter(f); setSelectedTicket(null); }}
              ...
              <div className="flex flex-col h-[550px]">
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="secondary" size="sm" onClick={() => handleStatus(selectedTicket, 'closed')} disabled={statusLoading} loading={statusLoading}>Close</Button>
                    <Button variant="secondary" size="sm" onClick={() => handleStatus(selectedTicket, 'open')} disabled={statusLoading} loading={statusLoading}>Re-open</Button>
                    <Button variant="secondary" size="sm" onClick={() => handleStatus(selectedTicket, 'pending')} disabled={statusLoading} loading={statusLoading}>Pending</Button>
                    <Button variant="danger" size="sm" onClick={() => handleDelete(selectedTicket)} disabled={statusLoading}>Delete</Button>
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {msgsLoading ? (
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 p-3 flex gap-2">
                  <textarea value={replyMsg} onChange={e => setReplyMsg(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleReply(); } }}
                    placeholder="Type your reply..." rows={2}
                    className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg text-sm dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
                  <Button onClick={handleReply} disabled={sending || !replyMsg} loading={sending}>
                    <Send className="h-4 w-4" /> Send
                  </Button>
                </div>
