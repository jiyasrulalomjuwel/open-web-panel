...
import { getChildAccount, getTickets, createTicket, getTicketMessages, replyTicket, updateTicketStatus, getAdminTickets, replyAdminTicket, deleteTicket } from '../lib/api';
...
export function Tickets() {
  ...
  const loadTickets = useCallback(() => {
    setLoading(true);
    getTickets().then(d => { setTickets(d || []); }).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const statusBadge = (s: string) => {
    const colors: Record<string, string> = { open: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300', closed: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400', replied: 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300' };
...
export function TicketManagement() {
  ...
  const loadTickets = useCallback(() => {
    setLoading(true);
    getAdminTickets().then(d => { setTickets(d || []); }).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const statusBadge = (s: string) => {
    const colors: Record<string, string> = { open: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300', closed: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400', replied: 'bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300' };
    return <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${colors[s] || 'bg-gray-100 text-gray-500'}`}>{s}</span>;
  };
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setConfirmDelete(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-sm mx-4 p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-2">Delete Ticket</h3>
