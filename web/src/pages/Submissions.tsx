...
type Submission = { id: number; account_id: number; form_type: string; metadata: Record<string, MetadataValue> | string | null; ip_address: string; created_at: string };
...
export function Submissions() {
  ...
  const filtered = filter
    ? subs.filter(s => s.form_type.toLowerCase().includes(filter.toLowerCase()) || JSON.stringify(s.metadata ?? '').toLowerCase().includes(filter.toLowerCase()))
    : subs;
  ...
  const formatMetadata = (m: Submission['metadata']): string => {
    if (m == null) return '';
    if (typeof m === 'string') {
      if (!m.trim()) return '';
      try { return JSON.stringify(JSON.parse(m), null, 2); } catch { return m; }
    }
    return Object.entries(m).map(([k, v]) => `${k}: ${String(v)}`).join('\n');
  };
