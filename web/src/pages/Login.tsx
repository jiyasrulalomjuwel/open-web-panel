...
import { login, loginChild, getUser, getAccessToken, getCaptcha, type CaptchaResult } from '../lib/api';
import { Server, ArrowRight, Eye, EyeOff, RefreshCw } from 'lucide-react';
...
export function Login() {
  ...
  const fetchCaptcha = useCallback(async () => {
    ...
    setCaptchaAnswer('');
    try {
      const result: CaptchaResult = await getCaptcha(mode === 'child');
      setCaptchaPrompt(result.challenge.prompt);
      setCaptchaSession(result.session_id);
    } catch {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setError('');
    if (!captchaSession || !captchaAnswer) {
    ...
    try {
      ...
      await fn(username, password, captchaSession, captchaAnswer);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-sm font-mono text-gray-700 dark:text-gray-200 select-none">
                  {captchaLoading ? (
                    <span className="text-gray-400">Loading...</span>
                  ) : (
                    captchaPrompt || 'Unable to load challenge'
                  )}
                </div>
                <button
                  type="button"
                  ...
                  className="p-2.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50 transition-colors"
