...
import { login, loginChild, getUser, getAccessToken } from '../lib/api';
import { Server, ArrowRight, Eye, EyeOff } from 'lucide-react';

function getPanelMode(): 'parent' | 'child' | 'both' {
  // A child-context path (e.g. /child/files) must always resolve to the Site
  // Owner login — never the admin login — otherwise users get bounced to the
  // wrong panel after their token expires.
  if (window.location.pathname.startsWith('/child')) return 'child';
  const port = window.location.port;
  if (port === '2086' || port === '9000') return 'parent';
  if (port === '2082' || port === '9001') return 'child';
  return 'both';
}

// On a shared port (80/443) both panels share one login page. Remember which
// panel the user was headed to so we can default to the right tab and return
// them to their original destination after a successful sign-in.
const REDIRECT_KEY = 'owp_login_redirect';
function getIntendedPath(): string {
...
function defaultModeFor(panelMode: 'parent' | 'child' | 'both'): 'parent' | 'child' {
  if (panelMode !== 'both') return panelMode;
  const intended = getIntendedPath();
  if (intended.startsWith('/child')) return 'child';
  return 'parent';
}
...
export function Login() {
  ...
  const [mode, setMode] = useState<'parent' | 'child'>(defaultModeFor(panelMode));
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Return the user to wherever they were headed (e.g. their site files
      // or the domains page) instead of always dropping them on the dashboard.
      const intended = getIntendedPath();
      clearLoginRedirect();
      if (intended.startsWith('/child')) {
        navigate(intended, { replace: true });
      } else {
        navigate(mode === 'parent' ? '/' : '/child/dashboard', { replace: true });
      }
    } catch (err: any) {
