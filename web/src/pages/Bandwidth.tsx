      ...
      <Card>
        ...
        {data?.days?.length > 0 ? (
          <div className="space-y-1">
            {(() => {
              const maxDay = Math.max(...(data.days || []).map((x: any) => x.bytes_out || 0), 1);
              return data.days.slice(0, 14).map((d: any) => (
                <div key={d.date} className="flex items-center gap-3 text-xs">
                  <span className="w-24 text-gray-500 dark:text-gray-400 truncate">{d.date}</span>
                  <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                    <div className="bg-blue-500 dark:bg-blue-400 h-full rounded-full" style={{ width: Math.min(100, (d.bytes_out || 0) / maxDay * 100) + '%' }} />
                  </div>
