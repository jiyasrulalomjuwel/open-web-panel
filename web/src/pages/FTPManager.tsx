...
import { Plus, X, AlertTriangle, Loader2, Trash2, Edit3, Eye, EyeOff, Server, AlertCircle } from 'lucide-react';
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        else if (showEdit) setShowEdit(null);
        else if (showCreate) setShowCreate(false);
      }
    };
    document.addEventListener('keydown', handler);
    return () => document.removeEventListener('keydown', handler);
  }, [showCreate, showEdit, confirmDelete]);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
              ...
              <Card>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1 ml-3 shrink-0">
                    ...
                    <button onClick={() => setConfirmDelete(a)} disabled={deleting === a.id} className="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 disabled:opacity-40">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md mx-4 p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Create FTP Account</h3>
              <button onClick={() => setShowCreate(false)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" aria-label="Close"><X className="h-5 w-5" /></button>
            </div>
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <motion.div
              ...
              className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-sm mx-4 p-6 shadow-xl"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-start gap-3 mb-4">
                <div className="p-2 rounded-full bg-red-50 dark:bg-red-900/30 shrink-0">
                  <AlertCircle className="h-5 w-5 text-red-500" />
                </div>
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900 dark:text-gray-100">Delete FTP Account</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Delete <strong>{confirmDelete.username}</strong>? This action cannot be undone.
                  </p>
                </div>
                <button onClick={() => setConfirmDelete(null)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 shrink-0" aria-label="Close"><X className="h-5 w-5" /></button>
              </div>
              <div className="flex justify-end gap-2">
                <Button variant="ghost" onClick={() => setConfirmDelete(null)}>Cancel</Button>
                <Button variant="danger" onClick={() => { const id = confirmDelete.id; setConfirmDelete(null); handleDelete(id); }}>
                  Delete
                </Button>
              </div>
            </motion.div>
      ...
      {showEdit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md mx-4 p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Edit FTP Account: {showEdit.username}</h3>
              <button onClick={() => setShowEdit(null)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" aria-label="Close"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
