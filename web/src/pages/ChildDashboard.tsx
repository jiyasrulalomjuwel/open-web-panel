...
import { getDiskUsage, getChildAccount, getChildDockerStats, getAvailablePhpVersions, getCurrentPhpVersion, selectPhpVersion } from '../lib/api';
import {
  Globe, Database, FolderOpen, Mail, HardDrive, Activity,
  Code2, CheckCircle, AlertCircle, ChevronRight,
  ArrowRight, Shield, Server, MemoryStick
} from 'lucide-react';
...
function StatNumber({ value }: { value: number }) {
  ...
  return <p className="text-[34px] font-bold text-gray-900 leading-none mt-2">{Math.round(animated)}</p>;
}
...
export function ChildDashboard() {
  ...
  const fetchDockerStats = useCallback(async () => {
    try {
      const data = await getChildDockerStats();
      setDockerStats(data);
    } catch {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getChildDockerStats().catch((e: any) => { console.error('Load docker stats:', e); return null; }),
    ...
    // Poll Docker stats every 20 seconds
  ...
  // Resource values from Docker container stats
  const hasDocker = dockerStats?.has_container;
  const du = hasDocker ? (dockerStats.disk_usage_mb || 0) : (disk?.size_mb || 0);
  const dh = hasDocker ? fmt(du) : (disk?.human || '0 B');
  const dl = account?.disk_limit_mb || 0;

  const bu = account?.bandwidth_used_mb || 0;
  const bl = account?.bandwidth_limit_mb || 0;

  const ru = hasDocker ? (dockerStats.ram_usage_mb || 0) : (account?.ram_used_mb || 0);
  const rl = hasDocker ? (dockerStats.ram_limit_mb || account?.ram_limit_mb || 0) : (account?.ram_limit_mb || 0);
  ...
  const quickActions = [
    { to: '/child/domains', label: 'Domains', icon: Globe, feature: null },
    { to: '/child/databases', label: 'Databases', icon: Database, feature: 'db' },
    { to: '/child/files', label: 'File Manager', icon: FolderOpen, feature: 'files' },
    { to: '/child/emails', label: 'Email', icon: Mail, feature: null },
    { to: '/child/ssl', label: 'SSL', icon: Shield, feature: null },
    { to: '/child/ftp', label: 'FTP', icon: Server, feature: 'ftp' },
  ].filter((a) => !a.feature || account?.features?.[a.feature] !== false);
  ...
  return (
    <motion.div className="max-w-7xl mx-auto space-y-8"
      ...
      {resourceWarning && (
        <motion.div variants={item} className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-card p-4">
      ...
      <div className="space-y-8">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
            {stats.map(({ label, value }) => (
              <motion.div key={label} variants={item} className="bg-white rounded-card border border-border-subtle shadow-soft overflow-hidden hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-200">
              ...
              <div className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{dh} of {dl === 0 ? 'Unlimited' : fmt(dl)} used{hasDocker ? <span className="inline-block ml-1.5 text-[10px] text-emerald-500 font-medium">live</span> : null}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {fmt(bu)} of {bl === 0 ? 'Unlimited' : fmt(bl)} used
                    </p>
                  </div>
                </div>
                <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
              ...
              <div className="p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{fmt(ru)} of {rl === 0 ? 'Unlimited' : fmt(rl)} used{hasDocker ? <span className="inline-block ml-1.5 text-[10px] text-emerald-500 font-medium">live</span> : null}</p>
                  </div>
                </div>
                <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
          ...
          <motion.div variants={item} className="bg-white rounded-card border border-border-subtle shadow-soft overflow-hidden">
            <div className="h-1 bg-gradient-to-r from-[#2563EB] to-[#3B82F6]" />
            <div className="p-6">
