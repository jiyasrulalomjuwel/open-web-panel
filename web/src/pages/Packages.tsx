...
export function Packages() {
  ...
  const emptyForm = { name: '', disk_mb: 1000, bandwidth_mb: 10000, max_db: 5, max_email: 10, max_ftp: 5, max_domains: 3, max_subdomains: 10, ssh_access: false, backup_enabled: true };
  const [form, setForm] = useState(emptyForm);

  const openEdit = (p: any) => {
    setEditId(p.id);
    setForm({ name: p.name, disk_mb: p.disk_mb, bandwidth_mb: p.bandwidth_mb, max_db: p.max_db, max_email: p.max_email, max_ftp: p.max_ftp, max_domains: p.max_domains, max_subdomains: p.max_subdomains, ssh_access: p.ssh_access, backup_enabled: p.backup_enabled });
    setShowModal('edit');
  };
  ...
  return (
    ...
    >
      ...
      {/* Create/Edit modal */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowModal(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-lg mx-4 p-6 shadow-xl max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[['Disk (MB)', 'disk_mb'], ['Bandwidth (MB)', 'bandwidth_mb'], ['Max Databases', 'max_db'], ['Max Emails', 'max_email'], ['Max FTP', 'max_ftp'], ['Max Domains', 'max_domains'], ['Max Subdomains', 'max_subdomains']].map(([label, key]) => (
                  <div key={key}>
