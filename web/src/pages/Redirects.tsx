...
import { getDomains, getAccessToken } from '../lib/api';
...
export function Redirects() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      request('/redirects'),
      getDomains().catch((e: any) => { console.error('Load domains:', e); return []; }),
    ]).then(([r, d]) => {
      if (!mounted) return;
      setRedirects(Array.isArray(r) ? r : []);
      setDomains(Array.isArray(d) ? d : []);
    }).catch((e: any) => { console.error('Load redirects:', e); toast('error', e?.message || 'Failed to load redirects'); }).finally(() => { if (mounted) setLoading(false); });
  ...
  const createRedirect = async () => {
    ...
    setSaving(true);
    try {
      const res = await request('/redirects', {
        method: 'POST',
        body: { domain_id: form.domain_id, source_path: form.source_path, target_url: form.target_url, type: form.redirect_type },
      });
  ...
  const deleteRedirect = async (id: number) => {
    ...
    } catch (e: any) { console.error('Delete redirect:', e); toast('error', e?.message || 'Failed to delete'); }
  };
