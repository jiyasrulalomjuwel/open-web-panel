...
export function SSLCertificates() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      getSSLCerts(),
      getDomains()
    ]).then(([c, d]) => { setCerts(c || []); setDomains(d || []); }).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  useEffect(() => {
    if (certs.some(c => c.status === 'issuing')) {
      const iv = setInterval(() => {
        getSSLCerts().then(setCerts).catch(() => {});
      }, 3000);
  ...
  return (
    ...
    >
      ...
      {showIssue && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowIssue(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-sm mx-4 p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-5">Issue SSL Certificate</h3>
