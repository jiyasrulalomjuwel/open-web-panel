...
import { Save, Settings2, Upload, Clock, Trash2, Loader2, Mail, Shield } from 'lucide-react';
...
export function Settings() {
  ...
  const handleSave = async () => {
    if (Object.keys(dirty).length === 0) { setSaved(true); setTimeout(() => setSaved(false), 2000); return; }
    setSaving(true);
    try {
      // Send only the edited keys so secret/unrelated stored values are never
      // echoed back wholesale to the server config.
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        <Card>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
            Suspended accounts are permanently deleted (files, databases, everything) after this many days. Set to 0 to disable auto-removal.
          </p>
          <div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <Card>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-medium text-gray-900 dark:text-gray-100">HTTPS Enforcement</h2>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
            When on, newly issued certificates automatically redirect HTTP to HTTPS. Per-domain switches stay on the Domains and SSL pages.
          </p>
          <label className="flex items-center gap-3 cursor-pointer">
            <button
              type="button"
              role="switch"
              aria-checked={(cfg.force_https_default ?? '1') !== '0'}
              onClick={() => handleChange('force_https_default', (cfg.force_https_default ?? '1') !== '0' ? '0' : '1')}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${(cfg.force_https_default ?? '1') !== '0' ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${(cfg.force_https_default ?? '1') !== '0' ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
            <span className="text-sm text-gray-700 dark:text-gray-300">Force HTTPS for newly issued certificates</span>
          </label>
        </Card>
      </motion.div>
