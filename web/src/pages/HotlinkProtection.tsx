...
export function HotlinkProtection() {
  ...
  useEffect(() => {
    ...
    request('/hotlink').then(r => {
      if (!mounted) return;
      const has = r !== undefined && r !== null && typeof r === 'object' && 'enabled' in r;
      setHasSettings(has);
      setEnabled(has ? r.enabled === true : false);
      setDomains(has && Array.isArray(r.allowed_domains)
        ? (r.allowed_domains.length ? r.allowed_domains : [''])
        : (r.allowed_domains || '').split(',').filter(Boolean) || ['']);
    }).catch((e: any) => console.error('Load hotlink:', e)).finally(() => { if (mounted) setLoading(false); });
  ...
  const save = async () => {
    ...
    try {
      await request('/hotlink', {
        method: 'POST',
        body: { enabled: Boolean(enabled), allowed_domains: domains.filter(Boolean).map(s => s.trim()) }
      });
      toast('success', enabled ? 'Hotlink protection enabled' : 'Hotlink protection disabled');
      setDomains(domains.filter(Boolean).map(s => s.trim()));
      setHasSettings(true);
    } catch (e: any) {
