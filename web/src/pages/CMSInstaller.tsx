...
export function CMSInstaller() {
  ...
  useEffect(() => {
    if (installs.some(i => i.status === 'downloading' || i.status === 'extracting' || i.status === 'configuring')) {
      const iv = setInterval(() => {
        getCMSInstalls().then(setInstalls).catch(() => {});
      }, 2000);
  ...
  return (
    ...
    >
      ...
      {showInstall && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 bg-black/40 overflow-y-auto" onClick={() => setShowInstall(false)}>
