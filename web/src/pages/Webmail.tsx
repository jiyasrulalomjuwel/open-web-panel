...
import { getEmails, readMessage, sendEmail, deleteMessage, getAccessToken } from '../lib/api';
...
export function Webmail() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    } catch (e: any) { console.error('Load accounts:', e); }
  ...
  const loadMessages = useCallback(async (acctId: number, fld: string) => {
    ...
    try {
      ...
      setMessages(msgs || []);
    } catch (e: any) { console.error('Load messages:', e); setMessages([]); }
  ...
  const openMessage = async (msgId: number) => {
    ...
    try {
      ...
      loadMessages(selectedAcct, folder);
    } catch (e: any) { console.error('Open message:', e); setError('Failed to load message'); }
  };
  ...
  return (
    ...
    >
      ...
      {readOnly && <ReadOnlyBanner text="Sending is disabled for your account — you can still read your messages." />}
