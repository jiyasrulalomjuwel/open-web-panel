...
func (s *Store) UpdateBandwidthUsage(id int, mb float64) error {
...
func (s *Store) GetStats() (map[string]interface{}, error) {
	...
	if err := s.db.QueryRow("SELECT COALESCE(CAST(SUM(disk_used_mb) AS INTEGER), 0) FROM accounts").Scan(&count); err != nil {
		return nil, err
	}
	stats["total_disk_used_mb"] = count

	var bwSum float64
	if err := s.db.QueryRow("SELECT COALESCE(SUM(bandwidth_used_mb), 0) FROM accounts").Scan(&bwSum); err != nil {
		return nil, err
	}
	stats["total_bandwidth_used_mb"] = bwSum

	if err := s.db.QueryRow("SELECT COALESCE(CAST(SUM(ram_used_mb) AS INTEGER), 0) FROM accounts").Scan(&count); err != nil {
		return nil, err
	}
