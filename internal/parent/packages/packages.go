...
func (s *Store) List() ([]Package, error) {
	rows, err := s.db.Query(`SELECT id, name, disk_mb, bandwidth_mb, ram_limit_mb, max_db, max_email, max_ftp,
	...
	for rows.Next() {
		var p Package
		if err := rows.Scan(&p.ID, &p.Name, &p.DiskMB, &p.BandwidthMB, &p.RamLimitMB, &p.MaxDB,
...
func (s *Store) GetByID(id int) (*Package, error) {
	var p Package
	err := s.db.QueryRow(`SELECT id, name, disk_mb, bandwidth_mb, ram_limit_mb, max_db, max_email, max_ftp,
		max_domains, max_subdomains, ssh_access, backup_enabled, is_default, created_at, updated_at
		FROM packages WHERE id = ?`, id).Scan(
		&p.ID, &p.Name, &p.DiskMB, &p.BandwidthMB, &p.RamLimitMB, &p.MaxDB,
		&p.MaxEmail, &p.MaxFTP, &p.MaxDomains, &p.MaxSubdomains, &p.SSHAccess,
...
func (s *Store) Create(req *CreatePackageRequest) (*Package, error) {
	...
	result, err := s.db.Exec(`INSERT INTO packages (name, disk_mb, bandwidth_mb, ram_limit_mb, max_db, max_email,
	...
	if err != nil || id == 0 {
		return nil, fmt.Errorf("create package: failed to get last insert id: %w", err)
	}
...
func (s *Store) Update(id int, req *CreatePackageRequest) (*Package, error) {
	...
	_, err := s.db.Exec(`UPDATE packages SET name=?, disk_mb=?, bandwidth_mb=?, ram_limit_mb=?, max_db=?,
		max_email=?, max_ftp=?, max_domains=?, max_subdomains=?, ssh_access=?,
		backup_enabled=? WHERE id=?`,
		req.Name, req.DiskMB, req.BandwidthMB, req.RamLimitMB, req.MaxDB, req.MaxEmail,
		req.MaxFTP, req.MaxDomains, req.MaxSubdomains, req.SSHAccess, backupEnabled, id)
	if err != nil {
